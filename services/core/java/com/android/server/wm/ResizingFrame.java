...
public class ResizingFrame {
    ...
    private void constructSurface(WindowManagerService service) {
        ...
        try {
            //mDimSurface = new SurfaceControl();
            mDimSurface = mService.makeSurfaceBuilder(new SurfaceSession()).
                setName(mName).
                setBufferSize(16, 16).
                setFormat(PixelFormat.TRANSLUCENT).
                setFlags(SurfaceControl.HIDDEN).build();
    ...
    public void startDragDraw() {
        ...
        mDimSurface.setBufferSize(fullscreenBounds.width(), fullscreenBounds.height());
    }

    public void stopDragDraw() {
        mDimSurface.setPosition(mBounds.left, mBounds.top);
        mDimSurface.setBufferSize(mBounds.width(), mBounds.height());
    }
